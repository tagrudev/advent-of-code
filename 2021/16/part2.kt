...
fun main() {
  challenge { lines ->
    ...
    println(part2(packets.first()))
  }
}

fun part2(packet: Packet) : Long {
  return when(packet.typeId) {
    4 -> (packet as LiteralPacket).data
    0 -> (packet as OperatorPacket).subpackages.sumOf { part2(it) }
    1 -> (packet as OperatorPacket).subpackages.map { part2(it) }.reduce { acc, l -> acc * l  }
    2 -> (packet as OperatorPacket).subpackages.minOf { part2(it) }
    3 -> (packet as OperatorPacket).subpackages.maxOf { part2(it) }
    5 -> (packet as OperatorPacket).subpackages.map { part2(it) }.reduce { acc, l -> if (acc > l) 1 else 0 }
    6 -> (packet as OperatorPacket).subpackages.map { part2(it) }.reduce { acc, l -> if (acc < l) 1 else 0 }
    7 -> (packet as OperatorPacket).subpackages.map { part2(it) }.reduce { acc, l -> if (acc == l) 1 else 0 }
    else -> 0L
  }
}
