...
fun main() {
  challenge { lines ->
    ...
    for (i in -500 until 500) { for (j in -500 until 500) { bruteForcePoints.add(Point(j, i)) } }
    ...
    var bruteForcedPoints = mutableListOf<Pair<Point, Int>>()
    bruteForcePoints.forEach { bruteForcePoint ->
      if (generateTrajectoryStepsFor(bruteForcePoint).take(500).any { it.within(gtx, ltx, gty, lty) }) {
        var trajectoryPoints = generateTrajectoryStepsFor(bruteForcePoint)
          .takeWhile { point -> !point.within(gtx, ltx, gty, lty) }
          .toList()

        if(trajectoryPoints.isEmpty()) return@forEach
        bruteForcedPoints.add(bruteForcePoint to trajectoryPoints.maxOf { it.y })
      }
    }

    println(bruteForcedPoints.maxOf { it.second })
  }
}
