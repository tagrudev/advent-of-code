...
# Part 1
all = input.split("\n").map do |sentence|
  z = sentence.split('').filter { _1.to_i != 0 }
  "#{z.first}#{z.last}".to_i
end
...
all =  input.split("\n").map do |sentence|
  matches = sentence.scan(/(?=(one|two|three|four|five|six|seven|eight|nine|\d))/).flatten

  if matches.size == 1
    digit = numbers.fetch(matches.first, matches.first)
    digit.concat(digit).to_i
  elsif matches.size > 1
    first = numbers.fetch(matches.first, matches.first)
    last = numbers.fetch(matches.last, matches.last)
    "#{first}#{last}".to_i
  else
    raise "... a problem has occurred with #{sentence}"
  end
end
